...
## 梦境退出与分段显示（2026-09-13，observe）

手机已对齐桌面关闭/归档确认，修复失败仍切页、旁白居中、分段同时显示与动画重播。证据与三面检查见 [修复说明](mobile/dream-wake-and-reveal.md)。真实手机连接运行中后端的软挽留、归档失败重试及长回复滚动尚未联调；建议分别验证正常退出、留下、坚持离开、网络中断及滚动回看。此次没有新增持久状态或端点，相邻后端和桌面仓库保持只读。
