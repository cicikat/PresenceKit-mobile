...
## 梦境与生活记录界面收敛（2026-09-13）

梦境动作/环境共用独立的 22px 水平外边距、14px 内边距和 0.7px 细框，底色不透明度在梦境 UI 设置中调节（默认 65%）。梦境输入栏采用主聊天的高度、内边距和发送样式，等待回复仍保留按钮与草稿；用户消息及等待气泡不显示身份/时间行。引用摘要增加灰色半透明底与外间距。

生活记录移除列表底部说明、结果来源说明和校正窗口的同意说明/节点地址。识别结果的「确认导入」覆盖编辑表单中识别出的标题、明细及备注；无匹配字段的文本进入备注，JSON 未匹配内容亦保留。分类与日期沿用用户选择，用户仍须保存才提交，取消不落盘。服务端识别证据不改写。

三面闭环：管理面继续拥有生活记录 enabled/effective、识别路由与审计；桌面无新增配置要求。导入沿既有 save → 原生队列 → capability/sync，保留 origin+owner、revision、operation_id、user_edited_fields、ack 和冲突处理。Dream HTTP、逐段 reveal、退出门控、通知与 TTL 不变；本次仅新增本机 appearance 偏好，无新业务队列或台账。真机视觉与触摸验收仍待进行。
