...
Android 同样支持导入/导出 JSON：在颜色预设页选择“导入主题 JSON”，或在某个预设上点“导出 mod”，使用系统文件选择/保存界面。导入始终新建副本，完整校验 schema 与 19 个颜色字段，上限 256 KiB；仅包含颜色，不含图片、字体、身份凭据或聊天。2026-09-11 已在真机导出用户“夜”预设，精确色值用于 `YxPalette.dark` 初始默认配置。
