...
## 裁决（维护者已拍板，2026-07-19）

采用「先拆、拆不动才调线」的混合方案：

1. **首选方向 1**：把 `4aa8a85` 拉长的弹窗 builder / 上传反馈方法整体抽成
   独立 widget 文件（沿用 `widgets/settings_editor_widgets.dart` 的既有拆分
   模式）。这些是纯 UI 文案态，抽 widget 是自然归宿，不进 controllers/。
2. **拆完仍超**（或某段确实无法自然抽离）才允许调 ratchet，上限一次性调到
   实际行数 +10 以内，并在测试文件里注释「i18n 迁移成本，YYYY-MM-DD，来源
   commit」。禁止无注释裸调数字——ratchet 每被无痛调高一次就死一分，
   它存在的意义是拦领域状态回流，不是拦本地化。
3. 方向 2 的「跟维护者确认」视为已完成，无需再询问。
