...
`AA打包安装到手机.bat` builds a debug APK and installs it to a connected device over adb. It hardcodes local `flutter`/`adb` paths at the top — edit those for your machine before running.

Current Android release builds use debug signing and are only suitable for personal testing, not distribution.
