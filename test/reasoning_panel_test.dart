...
void main() {
  testWidgets(
    'pending anchor loads canonical turn and toggles without a bubble',
    (tester) async {
      ...
      expect(tester.getSize(find.byType(TextButton)).height, lessThan(30));
      expect(tester.getSize(find.byType(TextButton)).width, lessThan(140));
      expect(
        tester
      ...
      await tester.pump();
      expect(
        find.text('Mira’s inner thoughts:\nA quiet moment.'),
        findsOneWidget,
      );
      await tester.pumpWidget(app('canonical-1', 'Ren'));
      expect(
        find.text('Ren’s inner thoughts:\nA quiet moment.'),
        findsOneWidget,
      );
